...
LONG CALLBACK CSkeletalViewerApp::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
		...
		{
			/*
		case IDC_BUTTONTrain:
			g_CSkeletalViewerApp.Nui_TrainGesturePattern(hWnd);
			break;
		case IDC_BUTTONLoad:
			SetDlgItemText( hWnd, IDC_DEBUG, L"Load Button pressed!");
			break;*/
		case IDC_BUTTONAdjustParameters:
			g_CSkeletalViewerApp.Nui_ParameterAdjustment(hWnd);
			break;
		case IDC_RADIOChrome:
			g_CSkeletalViewerApp.sendCommandsToApp = 2;
			break;
		case IDC_RADIOFirefox:
			//g_CSkeletalViewerApp.Nui_SetCommandSentTo( 1 );
			g_CSkeletalViewerApp.sendCommandsToApp = 1;
			break;
		case IDC_RADIOFromFile:
			g_CSkeletalViewerApp.sendCommandsToApp = 3;
			break;
		//case IDC_CHECKSendToBackground:
			//sendToBackground = IsDlgButtonChecked(hWnd, IDC_CHECKSendToBackground);
		//g_CSkeletalViewerApp.Nui_ParameterAdjustment(hWnd);
