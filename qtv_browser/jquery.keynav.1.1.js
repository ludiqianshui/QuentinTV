 ...
 */
  ...
  $.keynav.reg = function(e,onClass,offClass) {
	  ...
	  // VideoWall: initialize navigationdisabled property
	  e.navigationdisabled = false;
  ...
  $.keynav.quad = function(cur,fQuad) {
	  ...
	  for(i=0;i<kn.el.length;i++) {
		...
		// VideoWall: ignore disabled elements
		if (el.navigationdisabled) continue;
  ...
  $.keynav.goLeft = function () {
	  ...
	  var quad = $.keynav.quad(cur,function (dx,dy) { 
										...
										// VideoWall: narrow search triangle
										if((dy >= 0) && (10*Math.abs(dx) - dy) <= 0)
											return true;	
										else
											return false;
								   });
  ...
  $.keynav.goUp = function () {
	  ...
	  var quad = $.keynav.quad(cur,function (dx,dy) { 
										...
										// VideoWall: broaden search triangle
										if((dx >= 0) && (Math.abs(dy) - 10*dx) <= 0)
											return true;	
										else
											return false;
								   });
